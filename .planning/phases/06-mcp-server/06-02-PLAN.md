...
<task type="auto">
  <name>Task 1: Create component installer with directory mapping</name>
  <files>
    src/skill_retriever/mcp/installer.py
    tests/test_installer.py
  </files>
...
1. Create `src/skill_retriever/mcp/installer.py`:
   ...
   **class ComponentInstaller:**
   ```python
   def __init__(self, graph_store: GraphStore, target_dir: Path):
   ...
   def install(
       ...
       for comp_id in all_ids:
           node = self.graph_store.get_node(comp_id)
           if node is None:
               errors.append(f"Component not found: {comp_id}")
               continue

           # Get full metadata (need raw_content)
           # For now, construct minimal ComponentMetadata from node
           # Full implementation needs ComponentMetadataStore
           try:
               dest, tokens = install_component(
                   component=...,  # Need metadata store lookup
...
2. **Note on metadata lookup:** The current graph store only has GraphNode (id, type, label, embedding_id). Full ComponentMetadata with raw_content needs either:
   - A separate ComponentMetadataStore (file-based JSON)
   - Or store raw_content in node metadata

   For Plan 02, implement basic installer assuming metadata will be available via a `get_component_metadata(id) -> ComponentMetadata` function. Add a TODO for full metadata store integration.
...
3. Create `tests/test_installer.py`:
   ...
   - Test ComponentInstaller with mock graph_store
  ...
  <verify>
    ...
    - `uv run pytest tests/test_installer.py -v` passes
...
<task type="auto">
  <name>Task 2: Wire installer to MCP server and add metadata store</name>
  <files>
    src/skill_retriever/mcp/server.py
...
2. Update `src/skill_retriever/mcp/server.py`:
   ...
   - Update `get_pipeline()` to also initialize metadata_store
...
3. Update `src/skill_retriever/memory/__init__.py` to export MetadataStore.
...
4. Update `tests/test_mcp_server.py`:
  ...
  <verify>
    ...
    - `uv run ruff check src/skill_retriever/` passes
    - `uv run pyright src/skill_retriever/` passes
  </verify>
  <done>
    - MetadataStore provides component lookup by ID with raw_content
